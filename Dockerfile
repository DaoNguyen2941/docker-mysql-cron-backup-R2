...
COPY ["run.sh", "backup.sh", "restore.sh", "/delete.sh", "upload.sh" ,"/"]
RUN mkdir /backup && \
    chmod 777 /backup && \
    chmod 755 /run.sh /backup.sh /restore.sh /delete.sh /upload.sh && \
    touch /mysql_backup.log /mysql_backup_upload.log && \
    chmod 666 /mysql_backup.log /mysql_backup_upload.log
