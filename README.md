...
Run mysqldump to backup the database periodically using cron job manager in the container. Your backup is saved in `/backup` and can be uploaded to R2 cloudflare service. You can mount any directory on the host or docker volume in /backup. Otherwise, a docker volume will be created in the default location.
...
### ☁️ Upload Configuration (Optional)

- `ENABLE_UPLOAD`:  
  If set to `true`, the script will automatically upload backup files to Cloudflare R2 after the dump process completes.  
  Default: **disabled** (upload is skipped).

### ☁️ Cloudflare R2 Configuration

- `R2_ACCESS_KEY_ID`: Access Key for Cloudflare R2.  
- `R2_ACCESS_KEY_ID_FILE`: Path to file in the container containing the access key (supports Docker secrets).  
- `R2_SECRET_ACCESS_KEY`: Secret Key for Cloudflare R2.  
- `R2_SECRET_ACCESS_KEY_FILE`: Path to file containing the secret key (Docker secrets).  
- `R2_ACCOUNT_ID`: Account ID for Cloudflare R2.  
- `R2_ACCOUNT_ID_FILE`: Path to file containing the account ID (Docker secrets).  
- `R2_BUCKET`: Name of the Cloudflare R2 bucket to upload backups to.  
- `R2_BUCKET_FILE`: Path to file containing the bucket name (optional, Docker secrets).  
- `R2_ENDPOINT`: Custom endpoint for Cloudflare R2.  
  If `R2_ACCOUNT_ID` is set, the endpoint will be automatically generated as:  
  ```
